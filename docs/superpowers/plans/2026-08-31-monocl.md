...
Create a macOS App target named `MonoCl`, interface SwiftUI, language
Swift, bundle identifier `net.sinistral.monocl`. Then in build settings:
`MACOSX_DEPLOYMENT_TARGET = 26.0`, `SWIFT_VERSION = 6.0`,
`SWIFT_STRICT_CONCURRENCY = complete`. Add the three packages via
File > Add Package Dependencies > Add Local, and add all three libraries
to the target's Frameworks.

- [ ] **Step 5: Make it a menu bar app**
...
```bash
xcodebuild -scheme MonoCl -destination 'platform=macOS' build > "$SCRATCH/build.log" 2>&1; echo "exit=$?"
```

Expected: `exit=0`. Launch the built app and confirm: one dot in the menu
bar, no Dock icon, no window.

- [ ] **Step 8: Commit**

```bash
git add .gitignore MonoCl MonoCl.xcodeproj Packages
git commit -m "Add MonoCl project skeleton and local packages."
```
...
- Modify: `MonoCl.xcodeproj` (add a `MonoClTests` unit test target)
...
In Xcode: File > New > Target > Unit Testing Bundle, named `MonoClTests`,
with `MonoCl` as the host application. Set its Swift language mode to 6.
...
- [ ] **Step 5: Wire the Settings scene**
...
- [ ] Clicking Deny on the authorization dialog leaves the usage dots dim, shows "Keychain access denied" in the menu, and raises **no further dialog for at least three poll intervals** — verified by watching for three minutes at a 60 s interval
