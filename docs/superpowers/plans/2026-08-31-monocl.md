...
## Before any app-target task: regenerate, in that order

`project.yml` globs directories, and the glob is expanded into
`project.pbxproj` at generate time — not at build time. So a new file
under `MonoCl/` or `MonoClTests/` is invisible to `xcodebuild` until the
...
The order is always **create the file, then regenerate, then build or
test**.

**The dangerous failure is a false GREEN, not a false red.** Running
`xcodebuild test` against a stale project does not error on the missing
file — the project simply does not reference it, so the suite reports
`TEST SUCCEEDED` having silently run none of the new tests. A task can
therefore be marked complete with its tests never having executed once.
Getting the order wrong in the other direction is merely confusing: a
"cannot find X in scope" that reads like a code error rather than a
stale project.

So `exit=0` is NOT sufficient evidence that a new test ran. Check the
count moved:

```bash
grep -E 'Test run with [0-9]+ tests' "$SCRATCH/apptest.log"
```

A run that reports the same total as before you added a test means the
project is stale, whatever the exit code says. This is also why the TDD
step matters mechanically and not just as ritual: a test that has never
been *seen to fail* may never have been compiled at all.

Generation is deterministic, so a regenerate with no new files leaves
`MonoCl.xcodeproj` byte-identical and needs no commit — the project is
gitignored regardless.

This applies to Tasks 10 through 14. The three Swift packages need no
regeneration; `swift test` reads their manifests directly.
...
- [ ] `expiresAt` is interpreted correctly whether the source value is epoch seconds or epoch milliseconds (the live credential is milliseconds)
- [ ] `String(describing:)` of a `StoredCredential` does not contain the access token
- [ ] `KeychainCredentialReader` maps `errSecItemNotFound` to `.notFound`, and `errSecAuthFailed` / user cancellation / `errSecInteractionNotAllowed` to `.accessDenied`
- [ ] Any OTHER `OSStatus` maps to `.unexpected(status)`, carrying the raw status, and NOT to `.accessDenied`
...
public struct StoredCredential: Sendable, Decodable {
    ...
    /// The unit is undocumented.  A 13-digit value (epoch milliseconds)
    /// was observed on 2026-08-31; seconds are accepted too rather than
    /// resting on one observation.  The threshold is unambiguous either
    /// way: 10^11 seconds is the year 5138, so anything larger is
    /// milliseconds.
...
# The read-only constraint, mechanically.  Matching the open paren finds
# CALL SITES only: the reader's own doc comment names all three symbols
# in prose in order to assert their absence, so a bare-identifier grep
# can never return zero and would be a check that always fails.
grep -rn -E 'SecItem(Add|Update|Delete)\(' Packages/ | wc -l
```
...
- Create: `Packages/ClaudeUsage/Tests/ClaudeUsageTests/Fixtures/usage-bad-timestamp.json`
...
- [ ] `percent` equals `utilization` unchanged — the endpoint already reports 0 to 100
- [ ] `resetsAt` decodes an ISO-8601 string with six fractional-second digits and a `+00:00` offset
- [ ] `resetsAt` also decodes a `Z`-suffixed timestamp with no fractional seconds
- [ ] A `resets_at` that is not a timestamp throws rather than silently yielding a wrong date
...
These mirror the live response's real shape: `utilization` is a
percentage, `resets_at` is an ISO-8601 string, windows carry extra
dollar fields, and the payload has top-level keys MonoCl ignores.

`usage-both-windows.json` — the extra keys are present precisely so the
...
```json
{
  ...
  "seven_day_opus": { "utilization": 5.0, "resets_at": "2026-09-04T15:00:00Z" },
  "spend": { "limit_dollars": null },
  "member_dashboard_available": true
}
```

`usage-session-only.json` — also covers the no-fractional-seconds,
`Z`-suffixed timestamp form:

```json
{ "five_hour": { "utilization": 3.0, "resets_at": "2026-08-31T17:50:00Z" } }
```

`usage-no-windows.json`:

```json
{ "spend": { "limit_dollars": null }, "member_dashboard_available": true }
```

`usage-bad-timestamp.json`:

```json
{ "five_hour": { "utilization": 3.0, "resets_at": "not-a-timestamp" } }
```
...
- [ ] **Step 6: Confirm against the recorded contract**

Read `docs/superpowers/plans/2026-08-31-monocl.endpoint-contract.md` and
confirm the code above matches what was actually observed: `utilization`
taken unchanged, `resets_at` parsed as an ISO-8601 string. Both were
corrected in this plan AFTER the live probe; if the contract document
disagrees with the code, the contract document wins and you should stop
and report it.
...
- [ ] `CredentialError.notFound` yields `.credentialsNotFound`; `.accessDenied` yields `.keychainDenied`; `.unexpected` yields `.keychainUnavailable`
- [ ] `UsageFailure.stopsPolling` is true only for `.credentialsNotFound` and `.keychainDenied` — `.keychainUnavailable` keeps polling
...
- [ ] **Step 6: Confirm the header set against the recorded contract**

The contract document records that the endpoint returned 200 both with
and without `anthropic-beta: oauth-2025-04-20`, so the header is omitted
above. Confirm that is still what the document says; if it says the
header is required, add it back and report the discrepancy.
...
- Create: `Packages/PlatformStatus/Tests/PlatformStatusTests/Fakes.swift`
- Create: `Packages/PlatformStatus/Tests/PlatformStatusTests/PlatformStatusSourceTests.swift`
...
- [ ] A transport failure yields `.failure(.offline)`, asserted against a fake transport
- [ ] An undecodable body yields `.failure(.unexpectedResponse)`, asserted against a fake transport
- [ ] A non-200 status yields `.failure(.unexpectedResponse)`
- [ ] A 200 with a known indicator yields `.sample`, and the transport is called exactly once
...
- [ ] **Step 6: Write the source's own tests**

The decoding tests above cover `SummaryResponse` only. `fetch` has its
own outcomes — offline, undecodable, non-200 — and an acceptance
criterion with no test behind it is a claim rather than a check. A fake
transport is legitimate here for the same reason it is in `ClaudeUsage`:
this is the external-service network boundary, the one sanctioned place
for stubbing. The decoding, the response types and the mapping all stay
real.

```swift
// Packages/PlatformStatus/Tests/PlatformStatusTests/Fakes.swift
import Foundation
@testable import PlatformStatus
...
```swift
// Packages/PlatformStatus/Tests/PlatformStatusTests/PlatformStatusSourceTests.swift
import Foundation
import Indicators
import Testing
@testable import PlatformStatus
...
The last case matters more than it looks: the body is perfectly valid
JSON, so the test proves the status check runs BEFORE decoding rather
than a decode failure masquerading as a status failure.
...
- [ ] **Step 1: Note that the test target already exists**

There is no target to create and no Xcode GUI work in this task. Task 1
declared `MonoClTests` in `project.yml` and gave it a first test.

Adding files to `MonoClTests/` needs only a regenerate — but run it
AFTER the files exist, not now. See "Before any app-target task" above:
regenerating before the file is written accomplishes nothing and the
subsequent failure looks like a code error rather than a stale project.
The regenerate is folded into the run steps below.
...
- Modify: `MonoClTests/IndicatorStoreTests.swift` (add the display-order test)

**Acceptance Criteria:**
- [ ] `store.states` returns the three states in display order — session, weekly, platform — asserted with three distinguishable states so a transposition fails
- [ ] The produced image's `isTemplate` equals the spec's `isTemplate`
- [ ] Image height equals `NSStatusBar.system.thickness`
- [ ] Image width equals exactly 36 pt for three dots — 3 × 8 pt dots, 2 × 4 pt gaps, 2 pt inset each side
- [ ] The drawing handler actually produces a bitmap: `tiffRepresentation` is non-nil and yields an `NSBitmapImageRep` whose size equals the image's
...
- [ ] **Step 3: Pin the display order in the store**

`store.states` is the only place the dot order is encoded, and the order
is a stated product requirement — the reason the design uses one status
item rather than three is that macOS lets users freely reorder separate
items, so a light whose meaning depends on position must have that
position guaranteed. Nothing currently tests it: a future edit could
transpose two elements and every existing test would still pass, leaving
the session and weekly lights silently swapped.

Add this to `MonoClTests/IndicatorStoreTests.swift`, inside the existing
`IndicatorStoreTests` suite:
...
The three states are deliberately all different. An order test built
from identical states asserts nothing — it is the shape of test that
...
Regenerate first, since `MenuBarIcon.swift` and `MenuBarIconTests.swift`
are new files and the project's glob expands at generate time:
...
- [ ] The critical threshold cannot be observed below the warning threshold, by any caller, because the getter clamps on read
- [ ] Raising the warning threshold past the critical one masks the stored critical value rather than overwriting it: lowering the warning threshold again restores the user's original choice
- [ ] No unreachable branch claims to enforce the invariant — the enforcement is in exactly one place
...
**Files:**
- Create: `Scripts/check-fixture-drift.sh` (driver: fetches, then delegates)
- Create: `Scripts/compare-shape.py` (the comparison itself)

**Acceptance Criteria:**
- [ ] The shell driver passes `shellcheck` with no warnings
- [ ] Neither file is invoked by any test target, build phase, or CI configuration
- [ ] The comparison is DRIVEN FROM THE FIXTURES: the fixture files are read and their recorded types compared against the live response. No key name or type the check depends on is hard-coded in the script
- [ ] A consumed key present in the fixture but absent from the live response exits non-zero
- [ ] A consumed key whose JSON type differs between fixture and live exits non-zero — this is what catches `resets_at` reverting from string to number
- [ ] Keys present only in the LIVE response are ignored, not reported as drift
- [ ] An independently-optional window (`five_hour`, `seven_day`) missing from the live response is a NOTE, not drift, because the documented contract allows either to be absent
- [ ] A `utilization` value in `(0, 1]` is a NOTE, not drift, because a low percentage and a fraction are indistinguishable from one sample
- [ ] A `utilization` value outside `[0, 100]` exits non-zero
- [ ] Neither file ever prints the access token
...
The comparison is real logic, so it lives in Python rather than in bash
one-liners — which also removes the duplicated key extraction the first
draft carried.

Three properties make this comparison correct, and each was got wrong in
the first draft:

1. **It is driven from the fixtures.** A check whose expected key names
   are baked into the script does not compare the fixture to anything: the
   script's literals and the fixture can then drift apart silently, which
   is the very failure this tool exists to catch, displaced one level up.
2. **It is directional.** The live usage response carries far more keys
   than MonoCl consumes — roughly eighteen at the top level against two —
   and new ones appearing is routine. Only keys the FIXTURE records are
   required to persist.
3. **It compares TYPES, not just presence.** That is what catches
   `resets_at` reverting from an ISO-8601 string to an epoch number: both
   are present, both are non-null, and only the type differs.
...
Note what `--optional` buys: a window absent from the live response is a
NOTE rather than drift, because the documented contract allows either
window to be independently absent. A key missing from a window that IS
present is drift, because that is a genuine shape change. The first
draft conflated the two and treated both as a NOTE, so a real shape
change inside a present window would not have failed the check.

- [ ] **Step 3: Verify it**

```bash
chmod +x Scripts/check-fixture-drift.sh Scripts/compare-shape.py
shellcheck Scripts/check-fixture-drift.sh && echo "shellcheck-clean"
python3 -m py_compile Scripts/compare-shape.py && echo "python-syntax-ok"
```

- [ ] **Step 4: Prove the comparator actually discriminates**

The comparator can be exercised without any network access or keychain
read, by feeding it two local files. This is the evidence that matters:
a drift checker that cannot fail is worth nothing, and this is the one
part of the task that can be tested offline.

```bash
tmp="$(mktemp -d)"
cat > "$tmp/fixture.json" <<'JSON'
{ "five_hour": { "utilization": 47.0, "resets_at": "2026-08-31T17:50:00Z" } }
JSON

# 1. Identical shape -> pass
cp "$tmp/fixture.json" "$tmp/same.json"
python3 Scripts/compare-shape.py --fixture "$tmp/fixture.json" --live "$tmp/same.json"   --path five_hour --keys utilization resets_at; echo "identical exit=$?"

# 2. resets_at reverted to a number -> DRIFT
cat > "$tmp/epoch.json" <<'JSON'
{ "five_hour": { "utilization": 47.0, "resets_at": 1788198600 } }
JSON
python3 Scripts/compare-shape.py --fixture "$tmp/fixture.json" --live "$tmp/epoch.json"   --path five_hour --keys utilization resets_at; echo "epoch exit=$?"

# 3. A key dropped -> DRIFT
cat > "$tmp/dropped.json" <<'JSON'
{ "five_hour": { "utilization": 47.0 } }
JSON
python3 Scripts/compare-shape.py --fixture "$tmp/fixture.json" --live "$tmp/dropped.json"   --path five_hour --keys utilization resets_at; echo "dropped exit=$?"

# 4. Extra live keys -> NOT drift
cat > "$tmp/extra.json" <<'JSON'
{ "five_hour": { "utilization": 47.0, "resets_at": "2026-08-31T17:50:00Z",
                 "something_new": true }, "brand_new_top_level": 1 }
JSON
python3 Scripts/compare-shape.py --fixture "$tmp/fixture.json" --live "$tmp/extra.json"   --path five_hour --keys utilization resets_at; echo "extra exit=$?"

# 5. utilization 0.5 -> NOTE, exit 0 (ambiguous, not a claim)
cat > "$tmp/low.json" <<'JSON'
{ "five_hour": { "utilization": 0.5, "resets_at": "2026-08-31T17:50:00Z" } }
JSON
python3 Scripts/compare-shape.py --fixture "$tmp/fixture.json" --live "$tmp/low.json"   --path five_hour --keys utilization resets_at --percentage utilization; echo "low exit=$?"

# 6. utilization 1800 -> DRIFT (outside 0-100)
cat > "$tmp/huge.json" <<'JSON'
{ "five_hour": { "utilization": 1800.0, "resets_at": "2026-08-31T17:50:00Z" } }
JSON
python3 Scripts/compare-shape.py --fixture "$tmp/fixture.json" --live "$tmp/huge.json"   --path five_hour --keys utilization resets_at --percentage utilization; echo "huge exit=$?"

# 7. Window absent, marked optional -> NOTE, exit 0
echo '{}' > "$tmp/empty.json"
python3 Scripts/compare-shape.py --fixture "$tmp/fixture.json" --live "$tmp/empty.json"   --path five_hour --keys utilization resets_at --optional; echo "absent-optional exit=$?"

rm -rf "$tmp"
```

Expected exit codes, in order: `0, 1, 1, 0, 0, 1, 0`. Case 6 is the one
that matters most — 1800 is exactly what the multiplication bug this
project already made would have produced.

- [ ] **Step 5: Confirm it is not wired into the build**

```bash
grep -rn -E 'check-fixture-drift|compare-shape' --include='*.swift' --include='*.pbxproj' --include='*.yml' . | wc -l
```
...
```bash
git add Scripts/check-fixture-drift.sh Scripts/compare-shape.py
git commit -m "Drive the fixture drift check from the fixtures."
```
