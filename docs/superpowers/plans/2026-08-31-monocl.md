...
test**. Regenerating first, before the file exists, accomplishes
nothing and produces a confusing "cannot find X in scope" failure that
looks like a code error rather than a stale project. Generation is
deterministic, so a regenerate with no new files leaves
