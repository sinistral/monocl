...
import OSLog
    ...
    /// ---
    ///
    /// A keychain grant is bound to the requesting binary's signature, and
    /// an ad-hoc signature changes on every rebuild, so a freshly built
    /// MonoCl raises the authorization dialog again.  That dialog steals
    /// focus from whatever is in front, and an unattended run has nobody
    /// to answer it.  A launch that only needs the app to *hold* a
    /// credential can be handed one from here instead.
    ///
